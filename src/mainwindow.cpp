...
MainWindow::MainWindow(QWidget *parent)
    ...
    connect(btnSearch, &QPushButton::clicked, this, [this]() {
        ...
        ClientSessionManager::instance().addToHistory(historyEntry);
        
        // Log search to database for statistics
        if (!cr.excludedIngredients.isEmpty()) {
            ClientSessionManager::instance().logSearchToDatabase(cr.excludedIngredients.join(","));
        }
        ...
        if (foundTitles.isEmpty()) {
            resultsList->addItem("Рецепты не найдены.");
        } else {
            for (const QString& title : foundTitles) {
                QListWidgetItem *item = new QListWidgetItem("🍽️ " + title);
    ...
    connect(resultsList, &QListWidget::itemClicked, this, [this](QListWidgetItem *item) {
        ...
        // Remove "🍽️ " prefix properly
        if (fullText.startsWith("🍽️ ")) {
            title = fullText.mid(fullText.indexOf(' ') + 1);
        }
